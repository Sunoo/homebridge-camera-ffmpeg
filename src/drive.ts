...
function uploadPicture(folder, prefix, picture) {
  const drive = google.drive('v3');
  const d = new Date();
  const parsedUrl = url.parse(prefix.substr(prefix.search('http')), true, true);
  const name = prefix.replace(/ /g, '_') + '_' + d.toLocaleString().replace(/ /g, '_').replace(/,/g, '') + '.jpeg';
...
function uploadFile(auth) {
  ...
  const fetchPage = function (pageToken, pageFn, callback) {
    drive.files.list(
      {
        q: "mimeType='application/vnd.google-apps.folder' and name = 'Camera Pictures'",
        fields: 'nextPageToken, files(id, name)',
